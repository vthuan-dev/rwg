      ...
      <div className="p-6 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-extrabold text-slate-900">Hàng đợi Tín hiệu Gian lận ({links.length})</span>
          </div>
                  ...
                  links.map((link) => (
                    <tr key={link.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        <span className="font-bold text-red-700">{link.userAUsername || link.userAId}</span>
                        <span className="text-slate-400 mx-1 border-b border-dashed border-slate-300 px-1">↔</span>
                        <span className="font-bold text-red-700">{link.userBUsername || link.userBId}</span>
                      </td>
                      <td className="py-3.5 px-4">
                      ...
                      <td className="py-3.5 px-4 font-bold text-amber-700">{link.score} / 100</td>
                      <td className="py-3.5 px-4">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl max-w-md w-full p-6 flex flex-col gap-5 shadow-2xl relative">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <h3 className="text-base font-extrabold text-slate-900">Ghi Kết luận Gian lận</h3>
                <span className="text-xs text-slate-500 font-medium">Link ID: {concludeItem.id}</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <label className="text-xs font-bold text-slate-700">Kết luận Trạng thái</label>
              <select
                value={newStatus}
              ...
              <label className="text-xs font-bold text-slate-700">Lý do Kết luận * Bắt buộc</label>
              <textarea
                rows={3}
                required
                value={reason}
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-md w-full p-6 flex flex-col gap-5 shadow-2xl relative">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <h3 className="text-base font-extrabold text-slate-900">Nối Liên kết Thủ công</h3>
                <span className="text-xs text-slate-500 font-medium">Tạo liên kết gian lận bằng tay giữa 2 user</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <label className="text-xs font-bold text-slate-700">User ID A</label>
              <input
                type="text"
                value={userAInput}
              ...
              <label className="text-xs font-bold text-slate-700">User ID B</label>
              <input
                type="text"
                value={userBInput}
              ...
              <label className="text-xs font-bold text-slate-700">Lý do nối liên kết * Bắt buộc</label>
              <textarea
                rows={2}
                value={manualReason}
                onChange={(e) => setManualReason(e.target.value)}
                placeholder="Ghi rõ thông tin nghi vấn..."
                className="bg-slate-50 border border-slate-200 focus:border-red-500 rounded-xl p-3 text-xs text-slate-900 outline-none"
              />
            </div>
